...
gen_linux_bin ()
{
	...
	cp -ar ${K230_SDK_ROOT}/src/k230_canmv.dts hw/
	ROOTFS_BASE=`cat hw/k230_canmv.dts | grep initrd-start | awk -F " " '{print $4}' | awk -F ">" '{print $1}'`
	ROOTFS_SIZE=`ls -lt rootfs-final.cpio.gz | awk '{print $5}'`
	((ROOTFS_END= $ROOTFS_BASE + $ROOTFS_SIZE))
	ROOTFS_END=`printf "0x%x" $ROOTFS_END`
	sed -i "s/linux,initrd-end = <0x0 .*/linux,initrd-end = <0x0 $ROOTFS_END>;/g" hw/k230_canmv.dts

	${LINUX_BUILD_DIR}/scripts/dtc/dtc -I dts -q -O dtb hw/k230_canmv.dts  >k230.dtb;
